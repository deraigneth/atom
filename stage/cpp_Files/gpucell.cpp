...
#include "../header_Files/gpucell.h"

CPUCell *CPUCell::copyCellToDevice() {
    CPUCell *h_src, *d_dst;
    int err;
...
void CPUCell::copyCellFromDevice(CPUCell *d_src, CPUCell *h_dst) {
    static CPUCell *h_copy_of_d_src;
    static int first = 1;
    int err;

    if (first == 1) {
        first = 0;
        h_copy_of_d_src = new CPUCell;
...
CPUCell *CPUCell::allocateCopyCellFromDevice() {
    CPUCell *h_dst;
...
double CPUCell::compareToCell(Cell &d_src) {
